...
const MostSellProducts = () => {
  const [activeCategory, setActiveCategory] = useState("پردازنده");
  ...
  const categories = ["پردازنده", "کارت گرافیک", "حافظه رم", "مادربرد"];

  const filteredProducts = MostSellProductInfos.filter(
  ...
  return (
    <div className="w-[84%] m-auto mt-20" id="MostSellProducts">
      {/* Top */}
      <div className="flex lg:flex-row items-center justify-center flex-col lg:justify-between">
          ...
          <ul className="flex text-gray-400 gap-2 text-[18px] xl:ml-48 mx-auto">
            {categories.map((category) => (
              <li
                ...
                className={`rounded-md px-2.5 xl:px-4 cursor-pointer ${
                  activeCategory === category
                    ? "bg-red-600 text-white"
                    : "hover:text-white hover:bg-red-600"
                }`}
              >
                <a className="xl:text-[18px] lg:text-sm text-xs">{category}</a>
              </li>
            ))}
          </ul>
        </div>
        <div>
          <Link>
            ...
            >
              {showAll ? "پنهان کردن" : "مشاهده همه"}{" "}
              <IoMdArrowDropleft className="text-sm text-red-600 lg:flex hidden" />
            </h5>
          </Link>
        </div>
      </div>
