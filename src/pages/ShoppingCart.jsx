...
const ShoppingCart = () => {
  ...
  const totalOrderPrice = cart.reduce(
    (acc, product) => acc + product.oldPrice * product.quantity,
    0
  );
  const totalDiscount = cart.reduce(
    (acc, product) =>
      acc + (product.oldPrice - product.price) * product.quantity,
    0
  );
    ...
    <div className="overflow-hidden">
      ...
      <div className="pt-5 flex 2xl:flex-row flex-col justify-center items-center 2xl:justify-evenly w-[86%] mx-auto gap-5 mb-20 2xl:mb-60">
          ...
          <div className="mt-10 flex flex-col gap-4">
            {cart.length > 0 ? (
              cart.map((product) => (
                <div
                  key={product.id}
                  className="flex justify-between items-center w-[400px] sm:w-[550px] md:w-[650px] 2xl:w-[950px] h-36 mx-auto bg-gray-100 rounded-md"
                >
                  {/* Right */}
                  <div className="pr-10 flex gap-8 items-center">
                    <div>
                      <img
                        className="2xl:w-28 md:w-20 sm:w-16 w-12 h-12 sm:h-16 md:h-20 2xl:h-28"
                        src={product.img}
                        alt={product.title}
                      />
                    </div>
                    <div>
                      <h4 className="md:text-xl sm:text-base text-sm mt-4 font-medium">
                        {product.desc}
                      </h4>
                      <div className="mt-4">
                        {/* Rating Stars */}
                  ...
                  <div className="flex flex-col pl-6 leading-10">
                    <span className="line-through text-red-600 text-xs mb-2">
                      {product.oldPrice} تومان
                    </span>
                    <span className="bg-green-500 text-white rounded-lg w-[70px] md:w-[90px] text-xs h-7 md:text-sm font-bold items-center flex justify-center mb-1 cursor-pointer">
                      {product.percentage} تخفیف
                    </span>
                    <span className="md:text-lg text-xs sm:text-sm font-medium">
                      {product.price} تومان
                    </span>
                  </div>
                </div>
              ...
              <div className="w-[950px] mx-auto text-center">
        ...
        <div className="2xl:w-[300px] lg:w-[500px] md:w-[350px] w-[300px] h-[300px] border-[1px] shadow-sm shadow-red-600 border-red-600 rounded-md items-center flex flex-col justify-center gap-y-5">
          <div>
            <h3 className="text-red-600 font-medium text-lg xl:text-xl">
              جزئیات پرداخت
            </h3>
          </div>
          {/* Payment Details */}
          <div className="flex gap-8 mt-3">
            <div className="text-sm lg:text-lg 2xl:text-base font-medium text-gray-600">
              قیمت کل سفارشات:
            </div>
            <div className="text-sm font-medium lg:text-base text-gray-600">
              {totalOrderPrice} تومان
            </div>
          </div>
